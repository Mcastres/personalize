      ...
      <Thead>
        <Tr>
          ...
          <Th>
            <Typography variant="sigma" textColor="neutral600">
              {formatMessage({
                id: getTrad("Settings.locales.row.displayName"),
              })}
            </Typography>
          </Th>
          <Th>
            <Typography variant="sigma" textColor="neutral600">
              {formatMessage({
                id: getTrad("Settings.locales.row.default-locale"),
              })}
            </Typography>
          </Th>
          <Th>
      ...
      <Tbody>
        {locales.map((locale) => (
          <Tr
            key={locale.id}
            {...onRowClick({
              fn: () => locale.isDefault ? undefined : onEditLocale(locale),
