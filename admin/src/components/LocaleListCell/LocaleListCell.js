...
const mapToLocaleName = (locales, LocaleSlug) =>
  get(
    locales.find(({ slug }) => slug === LocaleSlug),
    'name',
    LocaleSlug
  );

const LocaleListCell = ({ localizations, locale: currentLocaleSlug, id }) => {
  const locales = useSelector(selectI18NLocales);
  const allLocalizations = [{ locale: currentLocaleSlug }, ...localizations];
  const localizationNames = allLocalizations.map(locale => locale.locale);
  const defaultLocale = locales.find(locale => locale.isDefault);
  const hasDefaultLocale = localizationNames.includes(defaultLocale.slug);
