...
const LocalePicker = () => {
  ...
  const [selected, setSelected] = useState(initialLocale?.slug || '');
  ...
  const displayedLocales = locales.filter(locale => {
    const canCreate = createPermissions.find(({ properties }) => {
      return get(properties, 'locales', []).includes(locale.slug);
    });
    const canRead = readPermissions.find(({ properties }) =>
      get(properties, 'locales', []).includes(locale.slug)
    );
  ...
  const handleClick = slug => {
    ...
    setQuery({
      plugins: { ...query.plugins, i18n: { locale: slug } },
    });
  };
  ...
  return (
    ...
    >
      {displayedLocales.map(locale => (
        <Option key={locale.id} id={`menu-item${locale.name || locale.slug}`} value={locale.slug}>
          {locale.name}
        </Option>
      ))}
    </Select>
