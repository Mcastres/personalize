...
describe('VariationSelect', () => {
  ...
  // it('only shows the variations that have not already been used', async () => {
  //   render();

  //   await waitFor(() =>
  //     expect(screen.queryByText('Loading the available variations...')).not.toBeInTheDocument()
  //   );
  //   fireEvent.click(screen.getByLabelText('Variations'));
  ...
  //   const onVariationChangeSpy = jest.fn();
  //   render({ onVariationChange: onVariationChangeSpy });
  ...
  //   expect(onVariationChangeSpy).toBeCalledWith({ code: 'fr', displayName: 'French (fr)' });
  // });
});
