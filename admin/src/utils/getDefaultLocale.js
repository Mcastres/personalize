...
const hasLocalePermission = (permissions, localeSlug) => {
  if (permissions) {
    const hasPermission = permissions.some(permission =>
      get(permission, 'properties.locales', []).includes(localeSlug)
    );
...
const getDefaultLocale = (ctPermissions, locales = []) => {
  ...
  if (hasLocalePermission(readPermissions, defaultLocale.slug)) {
    return defaultLocale.slug;
  }

  if (hasLocalePermission(createPermissions, defaultLocale.slug)) {
    return defaultLocale.slug;
  }
